...
pattern = r'rouge_(\d)_(.*?): (\d+\.\d+)'

for num_merges in list_num_merges:
    for selective in list_selective:
        list_files = glob.glob(root_dir + dataset + '/subworded_%d/%s/log/mei/decode_test*/ROUGE_results.txt')
        results = ['-'] * 9
        if len(list_files) == 0:
            pass
        else:
            try:
                ...
                print(list_files)
                exit(0)
            line = ' '.join([line.strip() for line in open(list_files[0], 'r').readlines()])
            for ngram, str_metric, str_value in re.findall(pattern, line):
                results[item2id[str_metric+ngram]] = str_value

        results.insert(0, '%dk,%s' % (num_merges // 1000, selective[:3]))
        target_file.write('\n' + ','.join(list_str_metric))
